...
sed -i 's/"max_instances":\ [^,]*,/"max_instances":\ 10,/g' $proc_config_1
...
# TODO: here
echo '\n\Evaluating the victim model on the test set (without config). \nOutputs will be saved to '$outdir_victim/test_results.jsonl'\n\n'
## outdir_victim=bin/runs/ruletaker/depth-5-base-2
python main.py \
    ...
    -o "{'trainer': {'cuda_device': "$cuda_device"}, 'validation_data_loader': {'batch_sampler': {'batch_size': 64, 'type': 'bucket'}}}" \
    --include-package ruletaker.allennlp_models
...
sed -i 's/"max_instances":\ [^,]*,/"max_instances":\ 10,/g' $proc_config_2
...
# outdir_attacker=bin/runs/adversarial/2021-12-07_08-22-00-keep
