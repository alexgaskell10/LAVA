...
def call_theorem_prover(
    ...
    if theorem_prover == "problog":
        ...
        try:
            ...
            elapsed_millisecs = end_millisecs - start_millisecs
            if print_log:
                print(f"!!!Encountered Exception at instance id {instance_id}, question id {question_id}: {e}")
            exception_name = str(type(e)).lstrip("<class '").rstrip("'>")

            ## We can infer label using CWA 
            assert assertion.polarity in ['+', '-']
            obtained_result = assertion.polarity == '-'

            return obtained_result, elapsed_millisecs, exception_name
        except (NegativeCycle, NonGroundProbabilisticClause) as e:
            end_millisecs = current_milli_time()
            elapsed_millisecs = end_millisecs - start_millisecs
            if print_log:
                print(f"!!!Encountered Exception at instance id {instance_id}, question id {question_id}: {e}")
            exception_name = str(type(e)).lstrip("<class '").rstrip("'>")
            return None, elapsed_millisecs, exception_name
    return obtained_result, None, None #return obtained_result, elapsed_millisecs, None
    ...
    for i, instance in enumerate(instances):
        ...
        for question_key in questions:
            ...
            engine_label, elapsed_millisecs, returned_exception = call_theorem_prover(
                theorem_prover, ix, question_key, theory, assertion, gold_label, print_log=False
            )
            engine_labels.append(engine_label)
...
def main():
    ...
    print(sys.argv)
    if len(sys.argv) < 2:
        with open('config/run_gen.yml', 'r') as f:
            args = argparse.Namespace(**yaml.load(f, Loader=yaml.SafeLoader))
    else:
        args = parse_args()
