...
from OpenAttack.attackers.textfooler import TextFoolerAttacker
...
from lava.openattack.custom_victim import Classifier

class CustomTextFoolerAttacker(TextFoolerAttacker):
    @property
    def TAGS(self):
    ...
    def attack(self, victim: Classifier, input : dict, goal: ClassifierGoal):
        """ * **clsf** : **Classifier** .
            * **x_orig** : Input input['context'].
        """
        # x_orig = input.lower()
        x_orig = input['context']
        question = input['question_text']

        orig_probs = victim.get_prob([[question, x_orig]])[0]
        ...
        leave_1_probs = victim.get_prob(
            [[question, self.tokenizer.detokenize(sentence)] for sentence in leave_1_texts])
        ...
        for idx, synonyms in synonyms_all:
            ...
            new_probs = victim.get_prob(
                [[question, self.tokenizer.detokenize(sentence)] for sentence in new_texts])
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                pred = victim.get_pred([[question, x_adv]])[0]
