...
victim=resources/runs/ruletaker/2021-12-12_17-38-38_roberta-large       # Victim used to train all attackers against
data_dir=data/rule-reasoning-dataset-V2020.2.4/depth-5/
outdir_attacker_base=resources/runs/ablate/$dt
mkdir -p $outdir_attacker_base
config_dir=resources/config/ablate/$dt
mkdir -p $config_dir
### USER INPUT (END) ###
...
do
    ...
    raw_config_1=resources/config/ablate/config.jsonnet
    ...
    raw_config_2=resources/config/ablate/test_config.jsonnet
