...
### USER INPUT (BEGIN) ###
max_instances=-1        # Number of data instances to use. -1 for all
cuda_device=1       # Currently only supports single GPU training
roberta_model="roberta-large"       # Victim model. Choices: roberta-base, roberta-large, distilroberta-base
outdir_victim=resources/runs/ruletaker/$dt'_'$roberta_model         # Victim destination dir
outdir_victim_retrain=resources/runs/ruletaker/$dt'_'$roberta_model'_'retrain         # Re-trained victim destination dir
outdir_attacker=resources/runs/adversarial/$dt'_'$roberta_model         # Attacker destination dir
data_dir=data/rule-reasoning-dataset-V2020.2.4/depth-5/                  # Location of data
### USER INPUT (END) ###

exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>'logs/main_flow-'$roberta_model'.out' 2>&1
...
raw_config_4=resources/config/ruletaker/ruletaker_adv_retraining.jsonnet
proc_config_4=resources/config/ruletaker/ruletaker_adv_retraining_$dt.jsonnet
...
raw_config_5=resources/config/ruletaker/ruletaker_adv_retraining_test.jsonnet
proc_config_5=resources/config/ruletaker/ruletaker_adv_retraining_test_$dt.jsonnet
